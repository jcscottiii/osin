...
func (s *Server) handleAuthorizationCodeRequest(w *Response, r *http.Request) *AccessRequest {
	...
	if ret.Client = getClient(auth, w.Storage, w, r); ret.Client == nil {
		return nil
	}

	// must be a valid authorization code
	var err error
	ret.AuthorizeData, err = w.Storage.LoadAuthorize(ret.Code, r)
	if err != nil {
		w.SetError(E_INVALID_GRANT, "")
		w.InternalError = err
		return nil
...
func (s *Server) handleRefreshTokenRequest(w *Response, r *http.Request) *AccessRequest {
	...
	var err error
	ret.AccessData, err = w.Storage.LoadRefresh(ret.Code, r)
	if err != nil {
		w.SetError(E_INVALID_GRANT, "")
		w.InternalError = err
		return nil
...
func (s *Server) FinishAccessRequest(w *Response, r *http.Request, ar *AccessRequest) {
	...
	if ar.Authorized {
		...
		if ret.AuthorizeData != nil {
			w.Storage.RemoveAuthorize(ret.AuthorizeData.Code, r)
		}

		// remove previous access token
		if ret.AccessData != nil {
			if ret.AccessData.RefreshToken != "" {
				w.Storage.RemoveRefresh(ret.AccessData.RefreshToken, r)
			}
			w.Storage.RemoveAccess(ret.AccessData.AccessToken, r)
		}
